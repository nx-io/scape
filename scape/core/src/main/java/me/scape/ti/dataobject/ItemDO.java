...
@NamedQueries({ 
	@NamedQuery(
			name = "Item.queryAccountFav", 
			query = "FROM ItemDO i WHERE i.user_id = ? AND i.type = ?"),
	})
public class ItemDO implements Serializable {
    ...
    private Integer media_count = 0;// 案例图片个数

    @Column(name = "comment_count", nullable = false)
    private Integer comment_count = 0;// 评论数

    @Column(name = "praise_count", nullable = false)
    private Integer praise_count = 0;// 赞数

    @Column(name = "like_count", nullable = false)
    private Integer like_count = 0;// 收藏数
